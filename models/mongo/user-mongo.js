...
import { cryptoConfig } from "../../config/crypto/index";
...
const userSchema = new mongoose.Schema({
    ...
    email:         {
        type:     String,
        unique:   "Такой aemail уже есть, если это вы, то войдите.",
...
userSchema.virtual('password')
    .set(function(password) {
        ...
        if (password) {
            this.salt = crypto.randomBytes(cryptoConfig.crypto.hash.length).toString('base64');
            this.passwordHash = crypto.pbkdf2Sync(password, this.salt, cryptoConfig.crypto.hash.iterations, cryptoConfig.crypto.hash.length, 'sha1');
        } else {
...
userSchema.methods.checkPassword = function(password) {
    ...
    return crypto.pbkdf2Sync(password, this.salt, cryptoConfig.crypto.hash.iterations, cryptoConfig.crypto.hash.length, 'sha1') == this.passwordHash;
};
